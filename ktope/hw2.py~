...
def sortGraph(graph):
    """Sort graph by not descending of nodes(number of connected to node nodes)"""
    ...
    while len(sorted)!=0:
        ...
        for nod in sorted:
            ...
            if nod in nodes and node not in sorted[nod]:
                ...
                exit()
            add=True
            #check that nodes colored on this iteration are not connected to nod
            for n in colored[j]:
                if n in sorted[nod]:
                    add=False
                    break
        ...
        for key in colored[j]:
            ...
            if key!=node:
                del sorted[key]
            #deleting all connections with this node
            for nod in sorted:
